...
| `LLM_PROVIDER` | `mock` | Reasoning provider: `openai-compatible`, `ollama`, `google` (Gemini), `openai`, `glm` (ZhipuAI), or `mock` |
...
## LLM providers
`llm.py` supports **OpenAI-compatible endpoints** (recommended for ROCm deployments such as llama-swap serving `Qwen3.8-27B-multimodal`), **Ollama** (local or cloud models), **Google Gemini**, **OpenAI**, **ZhipuAI GLM**, and a deterministic **mock** provider (default). Vision requests route to a local VLM when the active model is text-only, or natively to Gemini.
