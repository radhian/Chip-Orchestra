...
### AMD infrastructure split: Strix Halo agent + R9700 core

For the AMD infrastructure, use the same split-deployment idea but map the roles to the available hardware:

- **R9700 (`172.16.1.36`)** runs MySQL, Redis, `orchestrator-service`, `eda-service`, and the frontend.
- **Strix Halo (`172.16.1.10`)** runs `agent-service` only.
- The agent can call the R9700 OpenAI-compatible LLM endpoint at `http://172.16.1.36:8005/v1` by default, or the Strix Halo endpoint at `http://172.16.1.10:10000/v1` after `/v1/models` validation.
- Both hosts must mount the same workspace path, `/srv/chip-orchestra/workspaces`, so files generated by the agent are visible to EDA.

This AMD split is implemented in:

- `deploy/selfhosted-llm-rocm/AMD_DISTRIBUTED_ARCHITECTURE.md`
- `deploy/selfhosted-llm-rocm/docker-compose.r9700-core.yml`
- `deploy/selfhosted-llm-rocm/r9700-core.env.example`
- `deploy/selfhosted-llm-rocm/docker-compose.strix-agent.yml`
- `deploy/selfhosted-llm-rocm/strix-agent.env.example`
