...
rs.reset()
vecs = []
# power-on: rst_async_n=1, shreg flushes
for c in range(5):
    r = rs.step(1, 1)
...
for row in range(2):
    for col in range(3):
        pixel=row*10+col
        lb1=(row-1)*10+col if row>=1 else 0
        win,valid=w2.step(1,1,1,pixel,0,lb1,col,row)
        vecs.append({"inputs":{"clk":1,"rst_n":1,"shift_en":1,"pixel_in":pixel,
...
bus_cases = [
  ...
  (ADDR_UART_TXDATA,0,1,0,0,0x55,0),
  (ADDR_CGRA_CFG_BASE,0,1,0,0,0,0x77),
  (ADDR_START,1,0,0x01,0,0,0),
  (0x05,1,0,0xAB,0,0,0),
]
...
# ---------- uart_rx ----------
def send_byte_vec(rx, byte):
    vs=[]
    for _ in range(DIV):
        _,v=rx.step(1,1,0); vs.append(v)
    for b in range(8):
        bit=(byte>>b)&1
        for _ in range(DIV):
            _,v=rx.step(1,1,bit); vs.append(v)
    for _ in range(DIV):
        _,v=rx.step(1,1,1); vs.append(v)
    return vs
vecs=[]
for byte in [0xA5,0x00,0xFF,0x3C]:
    rx=UartRx(); rx.reset()
    vs=send_byte_vec(rx,byte)
    # record per-clock: rx_in and rx_valid, plus final rx_byte
    # We re-run capturing inputs
    rx=UartRx(); rx.reset()
...
for byte in [0x3C,0xFF,0xA5,0x00]:
    tx=UartTx(); tx.reset()
    tx.step(1,1,0,0)  # idle
    tx.step(1,1,0,0)  # idle
    tx.step(1,1,1,byte)  # pulse start
...
# ---------- top ----------
vecs=[]
# Use the functional model on the real input; record a sample of outputs
with open('context/chip_input_grid.json') as f:
    grid=json.load(f)
flat=[p for row in grid['pixels'] for p in row]
out=sobel_stream(flat)
# Record first 30 and last 30 output bytes as vectors (top is driven via UART,
# but the functional contract is the output stream). We record input pixels
# and expected output stream.
# For TB_GEN we provide a transaction-level vector: input frame + expected output
