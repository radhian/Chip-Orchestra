...
**NanoCGRA-Lite** is a minimal, ultra-compact **coarse-grained reconfigurable array (CGRA) soft-IP** implemented using the open **GF180MCU PDK** and generated through **Chip Orchestra**, targeting low-cost experimentation and deployment of reconfigurable hardware accelerators for resource-constrained edge devices. The canonical tapeout configuration integrates a **3×3 processing-element (PE) mesh** with nine lightweight compute elements, a compact **32-byte SRAM implemented as a 32×8-bit memory** for local data and intermediate results, and a simple **4-pin UART-only interface** (`clk`, `rst_n`, `uart_rx`, `uart_tx`) controlled by the packet-based `uart_bridge.v` FSM. The architecture is designed to provide configurable parallel computation with minimal silicon area and interface complexity, making it suitable for **TinyML and Edge AI inference, digital signal processing, sensor and IoT data processing, lightweight image and vision preprocessing, cryptographic primitives, and small mathematical or vector-processing kernels**. Workloads can be configured and executed through the UART interface, allowing an external host to load data, configure the CGRA, trigger computation, and retrieve results. By combining a small programmable compute fabric with local memory and a minimal communication interface, NanoCGRA-Lite explores the design space between a general-purpose microcontroller and a fixed-function accelerator while providing a practical open-source platform for **CGRA research, ASIC prototyping, hardware acceleration, and application-specific edge computing**. The results documented in this README correspond to the **verified EDA implementation flow for the canonical 3×3 PE + 32-byte SRAM configuration**.
