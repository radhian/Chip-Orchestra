...
def main() -> int:
    ...
    reachable_via_helpers = sorted(name for name in reachable if name.startswith("VIA_"))
    if reachable_via_helpers:
        errors.append("generated via helper cells remain hierarchical: " + ", ".join(reachable_via_helpers))
    else:
        print("generated via helpers=FLATTENED")
    if top is not None:
        bbox = top.bbox()
