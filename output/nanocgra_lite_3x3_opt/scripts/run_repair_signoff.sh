...
"$KLAYOUT_BIN" -zz -b \
  ...
  -rd report="$PKG/reports/signoff/drc_main_repair.lyrdb" \
  -rd topcell=NanoCGRA_Lite > "$PKG/reports/signoff/drc_main_repair.log" 2>&1
python3 - "$PKG/reports/signoff/drc_main_repair.lyrdb" <<'PY'
import sys
import xml.etree.ElementTree as ET
items = ET.parse(sys.argv[1]).getroot().find("items")
if items is None or len(items):
    raise SystemExit(f"main DRC is not clean: {len(items) if items is not None else 'missing items section'}")
print("PASS: main KLayout DRC has zero items")
PY
