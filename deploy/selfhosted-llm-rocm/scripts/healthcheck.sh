...
echo "== /v1/models @ $BASE =="
if ! models_json=$(curl -fsS "$BASE/v1/models"); then
  echo; echo "FAIL: server not reachable / not ready at $BASE"; exit 1
fi
printf '%s' "$models_json" | head -c 600
echo; echo

echo "== chat completion smoke test =="
if ! completion_json=$(curl -fsS "$BASE/v1/chat/completions" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer EMPTY" \
  -d "{\"model\":\"$MODEL\",\"messages\":[{\"role\":\"user\",\"content\":\"Reply with the single word: OK\"}],\"max_tokens\":16,\"temperature\":0}"); then
  echo; echo "FAIL: chat completion probe failed at $BASE"; exit 1
fi
printf '%s' "$completion_json" | head -c 1200
echo; echo
