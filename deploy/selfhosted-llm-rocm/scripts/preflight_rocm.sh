...
if command -v rocm-smi >/dev/null 2>&1; then
  if ! GPU_COUNT=$(rocm-smi --showid 2>/dev/null | awk '/^GPU/{count++} END{print count+0}'); then
    GPU_COUNT=0
  fi
...
AVAIL_GB=$(df -PBG "$HF_CACHE_DIR" 2>/dev/null | awk 'NR==2{gsub("G","",$4);print $4+0}')
note "[info]" "free at $HF_CACHE_DIR: ${AVAIL_GB:-?} GB (need ~${NEED_DISK_GB}GB for profile $HW_PROFILE)"
if [ "${AVAIL_GB:-0}" -lt "$NEED_DISK_GB" ]; then note "[warn]" "low disk for model weights"; fail=1; fi
