...
Just as important for this conversation: the **ROCm self-hosting path is planned around** this exact folder (`deploy/selfhosted-llm-rocm/`). The deployment shape is already defined as `agent-service` talking to an OpenAI-compatible vLLM-ROCm or ATOM server with no application code changes required, but the full deployment assets still need to land in the repo before we can describe that path as already present.
